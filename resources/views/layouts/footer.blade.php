<footer class="footer">
    <div class="footer__container">
        <div class="footer__content">
            <section class="left-content">
                <div class="social">
                    <h5 class="social__title">Dapatkan berita Ripox di:</h5>
                    <div class="social__content">
                        <a href="https://www.facebook.com/mrifrd.mrifrd " class="social__link">
                            <img src="{{asset('/img/socmed/socmed-facebook-H36.png')}}" class="social__icon" alt="">
                        </a>
                        <a href="https://youtube.com/@muhammadrifqifirdaus7415" class="social__link">
                            <img src="{{asset('/img/socmed/socmed-youtube-H36.png')}}" class="social__icon" alt="">
                        </a>
                        <a href="https://instagram.com/mrifrd_?igshid=YmMyMTA2M2Y=" class="social__link">
                            <img src="{{asset('/img/socmed/socmed-instagram-H36.png')}}" class="social__icon" alt="">
                        </a>
                    </div>
                </div>
                <div class="support">
                    <h5 class="support__title">Butuh Bantuan?</h5>
                    <a href="#" class="support__link">
                        <img src="{{asset('/img/support.png')}}" class="support__img" alt="">
                        <span class="support__text">Hubungi Kami</span>
                    </a>
                </div>
                <div class="country">
                    <img src="{{asset('/img/indonesia.png')}}" class="country__img" alt="">
                    <h5 class="country__title">Indonesia</h5>
                </div>
            </section>
            <section class="right-content">
                <div class="legal-content">
                    <a href="#" class="legal-content__text">Syarat & Ketentuan</a>
                    <a href="#" class="legal-content__text">Kebijakan Privasi</a>
                    <p class="legal-content__copyright">&copy; 2023 Ripox Store</p>
                </div>
            </section>
        </div>
    </div>
</footer>
